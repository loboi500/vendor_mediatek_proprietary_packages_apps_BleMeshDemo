...
public class ProvisionedDialogFragment extends DialogFragment implements TreeNode.TreeNodeClickListener, View.OnClickListener , AdapterView.OnItemClickListener{
    ...
    private static final String TAG = "ProvisionedDialogFragment";
    ...
    private String ctlDeltaUV = "0x7FFF";
    ...
    private boolean lightLightnessLightness = false;
    private boolean lightCTLLightnessFlag = false;
    private boolean lightCTLTemperatureFlag = false;
    private boolean lightCTLDeltaUVFlag = false;
    private boolean lightHSLLightnessFlag = false;
    private boolean lightHSLHueFlag = false;
    private boolean lightHSLSaturationFlag = false;
    private boolean friendshipFlag = false;
        ...
        if (node.isConfigSuccess()) {
            if(node.getNodeBearer() == MeshConstants.MESH_BEARER_ADV) {
                ...
                if(MyApplication.GattConnectStatus == MyApplication.PB_GATT_CONNECT) { //PB-GATT connect
                    ...
                    /*
                    if (node.getAddr() == MyApplication.GattConnectAddress) {
                        tvConnect.setEnabled(false);
                        tvDisconnect.setEnabled(true);
                    } else {
                        tvConnect.setEnabled(false);
            ...
            tvMessage.setEnabled(false); //if config fail, Message not click
            if(node.getNodeBearer() == MeshConstants.MESH_BEARER_ADV) {
                tvConnect.setVisibility(View.GONE);
                tvDisconnect.setVisibility(View.GONE);
            }else if(node.getNodeBearer() == MeshConstants.MESH_BEARER_GATT){
            ...
            case R.id.listConfig:
                switch (configDatas.get(pos)) {
                    case "Config Model Subscription Add":
                        elements = (MyApplication.nodeList).get(position).getElements();
                        if(elements.size() <= 0) {
                            Toast.makeText(getActivity(), "Element is null , please click Config Composition Data Get", Toast.LENGTH_SHORT).show();
                            break;
                        }
            ...
            case R.id.listMessage:
                ...
                switch (msg.getOpCode()) {
                    ...
                    case MeshConstants.MESH_MSG_GENERIC_ONOFF_SET: //generic onoff set
                        showSetGenericOnOffMessageDialog(msg);
                        break;
                    ...
                    case MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_SET: //light lightness set
                        showSetLightLightnessDialog(msg);
                        break;
    ...
    private void showSetGenericOnOffMessageDialog(MeshMessage msg) {
        ArrayList<String> appkeyIndexDatas = new ArrayList<String>();
        ArrayList<String> groupAddrDatas = new ArrayList<String>();
        for(int i = 0; i < msg.getBoundAppKeyList().size(); i++) {
            appkeyIndexDatas.add(MeshUtils.decimalToHexString("%04X",msg.getBoundAppKeyList().get(i)));
    ...
    private void showSetLightLightnessDialog(MeshMessage msg) {
        log("showSetLightLightnessDialog , opcode = " + msg.getOpCode());
        ArrayList<String> appkeyIndexDatas = new ArrayList<String>();
        ArrayList<String> groupAddrDatas = new ArrayList<String>();
        lightLightnessLightness = false;
        ...
        ArrayAdapter<String> groupAddrAdapter = new ArrayAdapter<String>(MyApplication.getApplication(),android.R.layout.simple_spinner_dropdown_item,groupAddrDatas);
        LayoutInflater inflater = getActivity().getLayoutInflater();
        View dialoglayout = inflater.inflate(R.layout.light_lightness_set_message, null);
        EditText etLightness = (EditText)dialoglayout.findViewById(R.id.etLightness);
        final AlertDialog mAlertDialog = new AlertDialog.Builder(getContext())
            ...
            .setPositiveButton("OK", new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface arg0, int arg1) {
                    if(mBluetoothMeshListener != null) {
                        int[] payload = new int[1];
                        payload[0] = Integer.parseInt(etLightness.getText().toString().trim());;
                        mBluetoothMeshListener.sendMessage(msg, position,
                            (int)MeshUtils.hexSrtingToDecimal(appkeyIndex),
                            (int)MeshUtils.hexSrtingToDecimal(groupAddr),
                            payload);
        ...
        final Button button = mAlertDialog.getButton(DialogInterface.BUTTON_POSITIVE);
        button.setEnabled(false);

        etLightness.addTextChangedListener(new TextWatcher(){
            ...
            public void afterTextChanged(Editable editable) {
                lightLightnessLightness = MeshUtils.isMatchInternal(editable.toString().trim(), 0x0001, 0xFFFF);
                if (lightLightnessLightness) {
    ...
    private void showSetLightCTLDialog(MeshMessage msg) {
        log("showSetLightCTLDialog , opcode = " + msg.getOpCode());
        ArrayList<String> appkeyIndexDatas = new ArrayList<String>();
        ArrayList<String> groupAddrDatas = new ArrayList<String>();
        lightCTLLightnessFlag = false;
        lightCTLTemperatureFlag = false;
        lightCTLDeltaUVFlag = false;
        ...
        ArrayAdapter<String> groupAddrAdapter = new ArrayAdapter<String>(MyApplication.getApplication(),android.R.layout.simple_spinner_dropdown_item,groupAddrDatas);
        LayoutInflater inflater = getActivity().getLayoutInflater();
        View dialoglayout = inflater.inflate(R.layout.light_ctl_set_message, null);
        EditText etCTLLightness = (EditText) dialoglayout.findViewById(R.id.etCTLLightness);
        EditText etCTLTemp = (EditText) dialoglayout.findViewById(R.id.etCTLTemp);
        EditText etCTLDeltaUV = (EditText) dialoglayout.findViewById(R.id.etCTLDeltaUV);
        final AlertDialog mAlertDialog = new AlertDialog.Builder(getContext())
            .setView(dialoglayout)
            .setPositiveButton("OK", new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface arg0, int arg1) {
                    if(mBluetoothMeshListener != null) {
                        ...
                        payload[0] = Integer.parseInt(etCTLLightness.getText().toString().trim());
                        payload[1] = Integer.parseInt(etCTLTemp.getText().toString().trim());
                        payload[2] = Integer.parseInt(etCTLDeltaUV.getText().toString().trim());
                        mBluetoothMeshListener.sendMessage(msg, position,
                            (int)MeshUtils.hexSrtingToDecimal(appkeyIndex),
                            (int)MeshUtils.hexSrtingToDecimal(groupAddr),
                            payload);
        ...
        etCTLLightness.addTextChangedListener(new TextWatcher(){
            ...
            @Override
            public void afterTextChanged(Editable editable) {
                lightCTLLightnessFlag = MeshUtils.isMatchInternal(editable.toString().trim(), 0x0001, 0xFFFF);
                if (lightCTLLightnessFlag && lightCTLTemperatureFlag && lightCTLDeltaUVFlag) {
                    button.setEnabled(true);
                } else {
                    button.setEnabled(false);
                }
        ...
        etCTLTemp.addTextChangedListener(new TextWatcher(){
            ...
            @Override
            public void afterTextChanged(Editable editable) {
                lightCTLTemperatureFlag = MeshUtils.isMatchInternal(editable.toString().trim(), 0x0320, 0x4E20);
                if (lightCTLLightnessFlag && lightCTLTemperatureFlag && lightCTLDeltaUVFlag) {
                    button.setEnabled(true);
                } else {
                    button.setEnabled(false);
        ...
        etCTLDeltaUV.addTextChangedListener(new TextWatcher(){
            ...
            @Override
            public void afterTextChanged(Editable editable) {
                lightCTLDeltaUVFlag = MeshUtils.isMatchInternal(editable.toString().trim(), 0x0000, 0xFFFF);
                if (lightCTLLightnessFlag && lightCTLTemperatureFlag && lightCTLDeltaUVFlag) {
                    button.setEnabled(true);
                } else {
                    button.setEnabled(false);
    ...
    private void showSetLightHSLDialog(MeshMessage msg) {
        log("showSetLightHSLDialog , opcode = " + msg.getOpCode());
        ArrayList<String> appkeyIndexDatas = new ArrayList<String>();
        ArrayList<String> groupAddrDatas = new ArrayList<String>();
        lightHSLLightnessFlag = false;
        lightHSLHueFlag = false;
        lightHSLSaturationFlag = false;
        ...
        ArrayAdapter<String> groupAddrAdapter = new ArrayAdapter<String>(MyApplication.getApplication(),android.R.layout.simple_spinner_dropdown_item,groupAddrDatas);
        LayoutInflater inflater = getActivity().getLayoutInflater();
        View dialoglayout = inflater.inflate(R.layout.light_hsl_set_message, null);
        EditText etHSLLightness = (EditText) dialoglayout.findViewById(R.id.etHSLLightness);
        EditText etHSLHue = (EditText) dialoglayout.findViewById(R.id.etHSLHue);
        EditText etHSLSaturation = (EditText) dialoglayout.findViewById(R.id.etHSLSaturation);
        final AlertDialog mAlertDialog = new AlertDialog.Builder(getContext())
            .setView(dialoglayout)
            .setPositiveButton("OK", new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface arg0, int arg1) {
                    if(mBluetoothMeshListener != null) {
                        int[] payload = new int[3];
                        payload[0] = Integer.parseInt(etHSLLightness.getText().toString().trim());
                        payload[1] = Integer.parseInt(etHSLHue.getText().toString().trim());
                        payload[2] = Integer.parseInt(etHSLSaturation.getText().toString().trim());
                        mBluetoothMeshListener.sendMessage(msg, position,
                            (int)MeshUtils.hexSrtingToDecimal(appkeyIndex),
                            (int)MeshUtils.hexSrtingToDecimal(groupAddr),
                            payload);
        ...
        etHSLLightness.addTextChangedListener(new TextWatcher(){
            ...
            @Override
            public void afterTextChanged(Editable editable) {
                lightHSLLightnessFlag = MeshUtils.isMatchInternal(editable.toString().trim(), 0x0001, 0xFFFF);
                if (lightHSLLightnessFlag && lightHSLHueFlag && lightHSLSaturationFlag) {
                    button.setEnabled(true);
                } else {
                    button.setEnabled(false);
                }
        ...
        etHSLHue.addTextChangedListener(new TextWatcher(){
            ...
            @Override
            public void afterTextChanged(Editable editable) {
                lightHSLHueFlag = MeshUtils.isMatchInternal(editable.toString().trim(), 0x0000, 0xFFFF);
                if (lightHSLLightnessFlag && lightHSLHueFlag && lightHSLSaturationFlag) {
                    button.setEnabled(true);
                } else {
                    button.setEnabled(false);
        ...
        etHSLSaturation.addTextChangedListener(new TextWatcher(){
            ...
            @Override
            public void afterTextChanged(Editable editable) {
                lightHSLSaturationFlag = MeshUtils.isMatchInternal(editable.toString().trim(), 0x0000, 0xFFFF);
                if (lightHSLLightnessFlag && lightHSLHueFlag && lightHSLSaturationFlag) {
                    button.setEnabled(true);
                } else {
                    button.setEnabled(false);
