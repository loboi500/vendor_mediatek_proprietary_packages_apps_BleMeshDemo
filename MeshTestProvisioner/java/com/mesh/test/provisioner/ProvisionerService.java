...
public class ProvisionerService extends Service{
    ...
    private CopyOnWriteArrayList<ResendConfigMsg> resendMsgList = new CopyOnWriteArrayList<>();
    ...
    public String frishipNetKey = null;
    public String frishipAppKey = null;
    public ArrayList<String> frishipAddrList = new ArrayList<>();
    public ArrayList<String> frishipNameList = new ArrayList<>();
    ...
    private int lightnessTID = 1; //only for LightLightness Set MSG
    private int ctlTemperatureTID = 1; //only for Light CTL Temperature Set MSG
    private int ctlTID = 1; //only for Light CTL Set MSG
    private int hslTID = 1; //only for Light HSL Set MSG
    ...
    private Timer sendPacketTimer = null;
    ...
    public void updateStorageData(int type) { //storage NetkeyMap, AppkeyMap, GroupAddr info
    ...
    public void updateNodeData(NodeData mNodeData, int type) {  //storage Node info
        log("updateNodeData");
        if(mBluetoothMeshListener != null) {
    ...
    private class ExTimerTask<T> extends TimerTask {
        ...
        private void gattProvisionEvent() {
            ...
            buffer.append(currentProvNode.getGattAddr());
            buffer.append(" , provision fail , time cost : 70s");
            log(buffer.toString());
        ...
        private void advProvisionEvent() {
            ...
            StringBuffer buffer = new StringBuffer();
            buffer.append(MeshUtils.intArrayToString(currentProvNode.getUUID(), true));
            buffer.append(" , provision fail , time cost : 60s");
            log(buffer.toString());
            if (mBluetoothMeshListener != null) {
        ...
        private void sendMessageTimeoutEvent() {
            ...
            if (isGroupSendMsg) { //if group send msg , need print ack node info
                MyApplication.groupSendMessageTime = System.currentTimeMillis() - MyApplication.groupSendMessageTime;
                log("sendMessageTimeoutEvent ---> group send message , not all node ack , ack node result as follows:");
                StringBuilder sb = new StringBuilder();
                log("sendMessageTimeoutEvent ---> ack node result as follows:");
                for (int i = 0; i < ackGroupSendMsgList.size(); i++) {
                    ...
                    if (uuid != null && gattAddress == null) {
                        sb.append("sendMessageTimeoutEvent ---> ack node ").append(i + 1).append(" = ").append(MeshUtils.intArrayToString(uuid, true));
                    }else if (uuid == null && gattAddress != null) {
                        sb.append("sendMessageTimeoutEvent ---> ack node ").append(i + 1).append(" = ").append(gattAddress);
                    }
                    log(sb.toString());
                ...
                boolean flag = false;
                int index = 1;
                log("sendMessageTimeoutEvent ---> not ack node result as follows:");
                ...
                for(int j = 0; j < cfSuccessNodeList.size(); j++) {
                    ...
                    if (!flag) {
                        ...
                        if (uuid != null && gattAddress == null) {
                            sb.append("sendMessageTimeoutEvent ---> not ack node ").append(index).append(" = ").append(MeshUtils.intArrayToString(uuid, true));
                            ...
                            sb.append("sendMessageTimeoutEvent ---> not ack node ").append(index).append(" = ").append(gattAddress);
                            if (unAckNode.getActiveStatus() == 0) {
                ...
                if (mBluetoothMeshListener != null) {
                    mBluetoothMeshListener.onNodeMsgRecieved(null);
                }
            }
        }
        ...
        private void sendMessageCountEvent() {
            ...
            if (msg.resendCnt >= SEND_MSG_DEFAULT_RESEND_TIMES) {
                log("sendMessageCountEvent ---> resendMessage " + msg.msgId + " for 4 times, stop resend");
                Node node = mNodes.get(msg.mDst);
                if (node != null) {
                    //stop resend timer task
                    msg.stopResendMsg();
                    ...
                    log("sendMessageCountEvent ---> not found node from mNodes");
                }
                return;
            }
            log("sendMessageCountEvent ---> resendMessage op = 0x" + Integer.toHexString(msg.mMsgOpCode) + ", msgId = " + msg.msgId + ", dst = 0x" + Integer.toHexString(msg.mDst));
            switch (msg.mMsgOpCode) {
                case MeshConstants.MESH_MSG_GENERIC_ONOFF_GET: //generic onoff get
                    mGenericOnOffClient.genericOnOffGet();
                    break;
                case MeshConstants.MESH_MSG_GENERIC_ONOFF_SET: //generic onoff set
                    mGenericOnOffClient.genericOnOffSet((msg.payload)[0], onOffTID++, msg.mTransitionTime, msg.mDelay);
                    break;
                case MeshConstants.MESH_MSG_GENERIC_ONOFF_SET_UNRELIABLE: //generic onoff set unack
                    mGenericOnOffClient.genericOnOffSetUnacknowledged((msg.payload)[0], onOffTID++, msg.mTransitionTime, msg.mDelay);
                    break;
                case MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_GET: //light lightness get
                    mLightnessClient.lightLightnessGet();
                    break;
                case MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_SET: //light lightness set
                    mLightnessClient.lightLightnessSet((msg.payload)[0], lightnessTID++, 0, 0);
                    break;
                case MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_SET_UNACKNOWLEDGED: //light lightness set unack
                    mLightnessClient.lightLightnessSetUnacknowledged((msg.payload)[0], lightnessTID++, 0, 0);
                    break;
                case MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_RANGE_GET: //light lightness range get
                    mLightnessClient.lightLightnessRangeGet();
                    break;

                case MeshConstants.MESH_MSG_LIGHT_CTL_GET: //light ctl get
                    mCTLClient.lightCtlGet();
                    break;
                case MeshConstants.MESH_MSG_LIGHT_CTL_SET: //light ctl set
                    mCTLClient.lightCtlSet((msg.payload)[0], (msg.payload)[1], (msg.payload)[2], ctlTID++, 0, 0);
                    break;
                case MeshConstants.MESH_MSG_LIGHT_CTL_SET_UNACKNOWLEDGED: //light ctl set unack
                    mCTLClient.lightCtlSetUnacknowledged((msg.payload)[0], (msg.payload)[1], (msg.payload)[2], ctlTID++, 0, 0);
                    break;
                case MeshConstants.MESH_MSG_LIGHT_CTL_TEMPERATURE_RANGE_GET: //light ctl temperature range get
                    mCTLClient.lightCtlTemperatureRangeGet();
                    break;
                case MeshConstants.MESH_MSG_LIGHT_HSL_GET: //Light HSL Get
                    mHSLClient.lightHslGet();
                    break;
                case MeshConstants.MESH_MSG_LIGHT_HSL_SET: //Light HSL Set
                    mHSLClient.lightHslSet((msg.payload)[0], (msg.payload)[1], (msg.payload)[2], hslTID++, 0, 0);
                    break;
                case MeshConstants.MESH_MSG_LIGHT_HSL_SET_UNACKNOWLEDGED: //Light HSL Set Unack
                    mHSLClient.lightHslSetUnacknowledged((msg.payload)[0], (msg.payload)[1], (msg.payload)[2], hslTID++, 0, 0);
                    break;
                case MeshConstants.MESH_MSG_LIGHT_HSL_RANGE_GET: //Light HSL Range Get
                    mHSLClient.lightHslRangeGet();
                    break;
                default:
                    log("sendMessageCountEvent ---> resendMsg UNKNOWN OPCODE: " + msg.mMsgOpCode);
                    break;
            }
            msg.resendCnt++;
        }
        ...
        public void scheduleResend() {
            ...
            try {
                if (nodeNumber < 0) {
                    ...
                    } else if (mMsgOpCode == MeshConstants.MESH_MSG_CONFIG_APPKEY_ADD){ //each packet cfg appkey add , mesh sdk need split 2 packet , 1 packet take 100ms * 3 = 300ms, total time = 600ms
                        this.resendConfigTimer.schedule(new ExTimerTask(0, this), 700, 700);
                    } else {
    ...
    public class ResendMsg {
        ...
        public ResendMsg(int msgId, int dst, int[] payload, int tid, int transitionTime, int delay, int msgOpCode) {
            this.msgId = msgId;
            this.mDst = dst;
    ...
    private void disable() {
        ...
        try {
        /*
            Iterator<ResendConfigMsg> it = resendMsgList.iterator();
            while (it.hasNext()) {
            ...
            */
            for(ResendConfigMsg msg:resendMsgList) {
                msg.stopResend();
                resendMsgList.remove(msg);
            }
        ...
        if (nodeList != null && nodeList.size() > 0) {
            for (int i = 0; i < nodeList.size(); i++) {
                ...
                nodeList.get(i).setFSStatus(0);
                nodeList.get(i).setFSOnCostTime(0);
                nodeList.get(i).setFSContOff(0);
                nodeList.get(i).setFSMaxContOff(0);
            }
            //updateStorageData(MyApplication.STORAGE_DATA_NODE);
        }
    }
    ...
    private void addResendMsg(Node node, int[] payload, int tid, int transitionTime, int delay, int msgOpCode, int retryCount) {
        log("addResendMsg, resendMsg = " + resendMsg);
        if (resendMsg != null) {
            resendMsg.stopResendMsg();
            resendMsg = null;
        }
        resendMsg = new ResendMsg(resendMsgID++, node.getAddr(), payload, tid, transitionTime, delay, msgOpCode);
    ...
    public void configMsgRelaySet(Node node, int relay, int rrc, int rris, boolean retry) {
        log("configMsgRelaySet to " + MeshUtils.decimalToHexString("%04X", node.getAddr()) + " , relay = " + relay + " , rrc = " + rrc + " , rris = " + rris);
        setConfigMeshMode(node);
        mConfigClient.setConfigMessageHeader(mPrimaryElement.getAddress(), node.getAddr(), defaultTTL, node.getCurrNetkeyIndex(),
    ...
    private void configClientMsgHandler(int modelHandle, BluetoothMeshAccessRxMessage msg) {
        log("configClientMsgHandler , modelHandle = " + modelHandle + " , msg = " + msg);
        ...
        switch (msg.getOpCode()) {
            ...
            case MeshConstants.MESH_MSG_CONFIG_MODEL_APP_STATUS: {
                ...
                if ((src == currentProvNode.getAddr()) && (currentProvNode.getConfigState() == MESH_NODE_CONFIG_STATE_MODEL_APP_BIND)) {
                    ...
                    if (i < modelSize) {
                        ...
                        //config relay set , not retry
                        configMsgRelaySet(currentProvNode, 1, 0, (int)(Math.random()*51), false);
            ...
            default: {
                log("not find match opcode , opcode = " + msg.getOpCode());
                break;
            }
        }
    }
        ...
        if (isGroupSendMsg) { // group send msg
            ...
            switch (msg.getOpCode()) {
                case MeshConstants.MESH_MSG_GENERIC_ONOFF_STATUS: {
                    /*
                     *  Present Onoff , 1 byte , The present value of the Generic Onoff state (0x00 is Off , 0x01 is On, 0x02~0xFF is Prohibited)
                     *  Target Onoff , 1 byte,  The target value of the Generic Onoff state(optional)
                     *  Ramaining Time, 1 byte
                     */
    ...
    private void groupSendMsgAck(int src, BluetoothMeshAccessRxMessage msg) {
        log("groupSendMsgAck() , src = " + MeshUtils.decimalToHexString("%04X", src) + " , opcode = " + MeshUtils.decimalToHexString("%04X", msg.getOpCode()));
        if (!cfSuccessNodeList.contains(src)) {
            log("groupSendMsgAck ---> this node isn't belong cfSuccessNodeList");
            return;
        }
        if (ackGroupSendMsgList.contains(src)) {
            log("groupSendMsgAck ---> ackGroupSendMsgList already contain src");
            return;
        }
        ...
        log("groupSendMsgAck ---> uuidOrGattAddr = " + uuidOrGattAddr);
        ...
        if (ackGroupSendMsgList.size() == cfSuccessNodeList.size()) {
            ...
            if (getMeshMode() != MeshConstants.MESH_MODE_STANDBY) {
                log("groupSendMsgAck ---> Set Mesh Mode Standby");
                setMeshMode(MeshConstants.MESH_MODE_STANDBY);
            }
            switch (msg.getOpCode()) {
                case MeshConstants.MESH_MSG_GENERIC_ONOFF_STATUS: { //Generic OnOff Status
                    int[] opcodes = {MeshConstants.MESH_MSG_GENERIC_ONOFF_GET,
                        MeshConstants.MESH_MSG_GENERIC_ONOFF_SET};
                    removeResendMsg(opcodes, groupSMNodeAddr);
                    break;
                }
                case MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_STATUS: { //Light Lightness Status
                    int[] opcodes = {MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_GET,
                        MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_SET};
                    removeResendMsg(opcodes, groupSMNodeAddr);
                    break;
                }
                case MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_RANGE_STATUS: { //Light Lightness Range Status
                    int[] opcodes = {MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_RANGE_GET};
                    removeResendMsg(opcodes, groupSMNodeAddr);
                    break;
                }
                case MeshConstants.MESH_MSG_LIGHT_CTL_STATUS: { //Light CTL Status
                    int[] opcodes = {MeshConstants.MESH_MSG_LIGHT_CTL_GET,
                        MeshConstants.MESH_MSG_LIGHT_CTL_SET};
                    removeResendMsg(opcodes, groupSMNodeAddr);
                    break;
                }
                case MeshConstants.MESH_MSG_LIGHT_CTL_TEMPERATURE_RANGE_STATUS: { //Light CTL Temperature Range Status
                    int[] opcodes = {MeshConstants.MESH_MSG_LIGHT_CTL_TEMPERATURE_RANGE_GET};
                    removeResendMsg(opcodes, groupSMNodeAddr);
                    break;
                }
                case MeshConstants.MESH_MSG_LIGHT_HSL_STATUS: { //Light HSL Status
                    int[] opcodes = {MeshConstants.MESH_MSG_LIGHT_HSL_GET,
                        MeshConstants.MESH_MSG_LIGHT_HSL_SET};
                    removeResendMsg(opcodes, groupSMNodeAddr);
                    break;
                }
                case MeshConstants.MESH_MSG_LIGHT_HSL_RANGE_STATUS: { //Light HSL Range Status
                    int[] opcodes = {MeshConstants.MESH_MSG_LIGHT_HSL_RANGE_GET};
                    removeResendMsg(opcodes, groupSMNodeAddr);
                    break;
                }
                default:
            ...
            log("groupSendMsgAck ---> group send message , all node ack , result as follows:");
            StringBuilder sb = new StringBuilder();
            for (int i = 0; i < ackGroupSendMsgList.size(); i++) {
                Node ackNode = getNodeByAddr(ackGroupSendMsgList.get(i));
                int[] uuid = ackNode.getUUID();
                String gattAddress = ackNode.getGattAddr();
                if (uuid != null && gattAddress == null) {
                    sb.append("groupSendMsgAck ---> ack node ").append(i + 1).append(" = ").append(MeshUtils.intArrayToString(uuid, true));
                }else if (uuid == null && gattAddress != null) {
                    sb.append("groupSendMsgAck ---> ack node ").append(i + 1).append(" = ").append(gattAddress);
                }
                log(sb.toString());
                sb.delete(0, sb.length());
            }
    ...
    private void lightnessClientMsgHandler(int modelHandle, BluetoothMeshAccessRxMessage msg) {
        int msgOpCode = msg.getOpCode();
        int src = msg.getSrcAddr();
        log("lightnessClientMsgHandler , msgOpCode = " + MeshUtils.decimalToHexString("%04X", msgOpCode) +
                " , src = " + MeshUtils.decimalToHexString("%04X", src));
        if (isGroupSendMsg) { // group send msg
            groupSendMsgAck(src, msg);
        } else { // single node send msg
            if (ackNodeSendMsgList.contains(src)) {
                log("lightnessClientMsgHandler ---> already contain src");
                return;
            }
            MyApplication.sendMessageTime = System.currentTimeMillis() - MyApplication.sendMessageTime;
            ackNodeSendMsgList.add(src);
            if (getMeshMode() != MeshConstants.MESH_MODE_STANDBY) {
                log("lightnessClientMsgHandler ---> Set Mesh Mode Standby");
                setMeshMode(MeshConstants.MESH_MODE_STANDBY);
            }
            if (getTimerById(currentProvNode.getSendMessageTimerID()) != null) {
                getTimerById(currentProvNode.getSendMessageTimerID()).cancel(); // cancel sendMessageTimer
            ...
            switch (msg.getOpCode()) {
                case MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_STATUS: {
                    /*
                     *  Present Lightness , 2 byte , The present value of the Light Lightness Actual state
                     *  Target Lightness , 2 byte,  The target value of the Light Lightness Actual state(optional)
                     *  Ramaining Time, 1 byte
                     */
                    int[] opcodes = {MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_GET, MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_SET, MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_SET_UNACKNOWLEDGED};
                    removeResendMsg(opcodes, src);
                    if (msg.getBuffer().length == 2) {
                        log("lightnessClientMsgHandler ---> Present Lightness = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[1], msg.getBuffer()[0])));
                    } else if (msg.getBuffer().length == 4) {
                        log("lightnessClientMsgHandler ---> Present Lightness = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[1], msg.getBuffer()[0])) + " , Target Lightness = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[3], msg.getBuffer()[2])));
                    } else if (msg.getBuffer().length == 5) {
                        log("lightnessClientMsgHandler ---> Present Lightness = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[1], msg.getBuffer()[0])) + " , Target Lightness = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[3], msg.getBuffer()[2])) + " , RemaningTime = " +
                            MeshUtils.decimalToHexString("%02X", msg.getBuffer()[4]));
                    } else {
                        log("lightnessClientMsgHandler ---> No valid data , msg.getBuffer().length = " + msg.getBuffer().length);
                        return;
                    }
                    if (mBluetoothMeshListener != null) {
                        mBluetoothMeshListener.onNodeMsgRecieved(msg);
                ...
                case MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_RANGE_STATUS: {
                    /*
                     *  Status Code , 1 byte , Status Code for the requesting message
                     *  Range Min , 2 byte,  The value of the Lightness Range Min field of the Light Lightness Range state
                     *  Range Max , 2 byte,  The value of the Lightness Range Max field of the Light Lightness Range state
                     */
                    int[] opcodes = {MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_RANGE_GET, MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_RANGE_SET, MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_RANGE_SET_UNACKNOWLEDGED};
                    removeResendMsg(opcodes, src);
                    if (msg.getBuffer().length != 5) {
                        log("lightnessClientMsgHandler ---> No valid data , msg.getBuffer().length = " + msg.getBuffer().length);
                        return;
                    }
                    int lightLightnessRangeMin = MeshUtils.getCombineNuber(msg.getBuffer()[2], msg.getBuffer()[1]);
                    int lightLightnessRangeMax = MeshUtils.getCombineNuber(msg.getBuffer()[4], msg.getBuffer()[3]);
                    if (lightLightnessRangeMin == -1 || lightLightnessRangeMax == -1) {
                        log("lightnessClientMsgHandler ---> No valid data");
                        for (int i = 0; i < msg.getBuffer().length ; i++) {
                            log("lightnessClientMsgHandler ---> msg.getBuffer()[" + i + "] = " + MeshUtils.decimalToHexString("%04X", msg.getBuffer()[i]));
                        }
                        return;
                    }
                    log("lightnessClientMsgHandler , Status Code = " +
                        MeshUtils.decimalToHexString("%02X", msg.getBuffer()[0]) + " , RangeMin = " +
                        MeshUtils.decimalToHexString("%04X", lightLightnessRangeMin) + " , RangeMax = " +
                        MeshUtils.decimalToHexString("%04X", lightLightnessRangeMax));
                    if (mBluetoothMeshListener != null) {
                        mBluetoothMeshListener.onNodeMsgRecieved(msg);
                    }
                    break;
    ...
    private void ctlClientMsgHandler(int modelHandle, BluetoothMeshAccessRxMessage msg) {
        int msgOpCode = msg.getOpCode();
        int src = msg.getSrcAddr();
        log("ctlClientMsgHandler, msgOpCode = " + MeshUtils.decimalToHexString("%04X", msgOpCode) +
                " , src = " + MeshUtils.decimalToHexString("%04X", src));
        if (isGroupSendMsg) { // group send msg
            groupSendMsgAck(src, msg);
        } else { // single node send msg
            if (ackNodeSendMsgList.contains(src)) {
                log("ctlClientMsgHandler ---> already contain src");
                return;
            }
            MyApplication.sendMessageTime = System.currentTimeMillis() - MyApplication.sendMessageTime;
            ackNodeSendMsgList.add(src);
            if (getMeshMode() != MeshConstants.MESH_MODE_STANDBY) {
                log("ctlClientMsgHandler ---> set Mesh Mode Standby");
                setMeshMode(MeshConstants.MESH_MODE_STANDBY);
            }
            if (getTimerById(currentProvNode.getSendMessageTimerID()) != null) {
                getTimerById(currentProvNode.getSendMessageTimerID()).cancel(); // cancel sendMessageTimer
            ...
            switch (msg.getOpCode()) {
                case MeshConstants.MESH_MSG_LIGHT_CTL_TEMPERATURE_STATUS: { //light CTL Temperatura ack
                    int[] opcodes = {MeshConstants.MESH_MSG_LIGHT_CTL_TEMPERATURE_GET,
                        MeshConstants.MESH_MSG_LIGHT_CTL_TEMPERATURE_SET,
                        MeshConstants.MESH_MSG_LIGHT_CTL_TEMPERATURE_SET_UNACKNOWLEDGED};
                    removeResendMsg(opcodes, src);
                    if (msg.getBuffer().length == 2) {
                        log("ctlClientMsgHandler ---> PresentLightness = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[1], msg.getBuffer()[0])));
                    } else if (msg.getBuffer().length == 4) {
                        log("ctlClientMsgHandler ---> PresentLightness = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[1], msg.getBuffer()[0])) + " , targetLightness = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[3], msg.getBuffer()[2])));
                    } else if (msg.getBuffer().length == 5) {
                        log("ctlClientMsgHandler ---> PresentLightness = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[1], msg.getBuffer()[0])) + " , TargetLightness = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[3], msg.getBuffer()[2])) + " , RemaningTime = " +
                            MeshUtils.decimalToHexString("%04X", msg.getBuffer()[4]));
                    } else {
                        log("ctlClientMsgHandler ---> No valid data , msg.getBuffer().length = " + msg.getBuffer().length);
                        return;
                    }
                    if (mBluetoothMeshListener != null) {
                        mBluetoothMeshListener.onNodeMsgRecieved(msg);
                ...
                case MeshConstants.MESH_MSG_LIGHT_CTL_TEMPERATURE_RANGE_STATUS: { //light CTL Temperatura Range ack
                    int[] opcodes = {MeshConstants.MESH_MSG_LIGHT_CTL_TEMPERATURE_RANGE_GET,
                        MeshConstants.MESH_MSG_LIGHT_CTL_TEMPERATURE_RANGE_SET};
                    removeResendMsg(opcodes, src);
                    if (msg.getBuffer().length != 5) {
                        log("ctlClientMsgHandler ---> No valid data , msg.getBuffer().length = " + msg.getBuffer().length);
                        return;
                    }
                    int lightCTLTemperatureRangeMin = MeshUtils.getCombineNuber(msg.getBuffer()[2], msg.getBuffer()[1]);
                    int lightCTLTemperatureRangeMax = MeshUtils.getCombineNuber(msg.getBuffer()[4], msg.getBuffer()[3]);
                    if (lightCTLTemperatureRangeMin == -1 || lightCTLTemperatureRangeMax == -1) {
                        log("ctlClientMsgHandler ---> No valid data");
                        for (int i = 0; i < msg.getBuffer().length ; i++) {
                            log("ctlClientMsgHandler ---> msg.getBuffer()[" + i + "] = " + MeshUtils.decimalToHexString("%04X", msg.getBuffer()[i]));
                        }
                        return;
                    }
                    log("ctlClientMsgHandler ---> Status Code = " +
                        MeshUtils.decimalToHexString("%02X", msg.getBuffer()[0]) + " , RangeMin = " +
                        MeshUtils.decimalToHexString("%04X", lightCTLTemperatureRangeMin) + " , RangeMax = " +
                        MeshUtils.decimalToHexString("%04X", lightCTLTemperatureRangeMax));
                    if (mBluetoothMeshListener != null) {
                        mBluetoothMeshListener.onNodeMsgRecieved(msg);
                    }
                    break;
                }
                case MeshConstants.MESH_MSG_LIGHT_CTL_DEFAULT_STATUS: { //light CTL Default ack
                    int[] opcodes = {MeshConstants.MESH_MSG_LIGHT_CTL_DEFAULT_GET,
                        MeshConstants.MESH_MSG_LIGHT_CTL_DEFAULT_SET,
                        MeshConstants.MESH_MSG_LIGHT_CTL_DEFAULT_SET_UNACKNOWLEDGED};
                    removeResendMsg(opcodes, src);
                    break;
                }
                case MeshConstants.MESH_MSG_LIGHT_CTL_STATUS: { //light CTL  ack
                    int[] opcodes = {MeshConstants.MESH_MSG_LIGHT_CTL_GET,
                        MeshConstants.MESH_MSG_LIGHT_CTL_SET,
                        MeshConstants.MESH_MSG_LIGHT_CTL_SET_UNACKNOWLEDGED};
                    removeResendMsg(opcodes, src);
                    if (msg.getBuffer().length == 2) {
                        log("ctlClientMsgHandler ---> Present CTL Lightness = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[1], msg.getBuffer()[0])));
                    } else if (msg.getBuffer().length == 4) {
                        log("ctlClientMsgHandler ---> Present CTL Lightness = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[1], msg.getBuffer()[0])) + " , Present CTL Temperature = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[3], msg.getBuffer()[2])));
                    } else if (msg.getBuffer().length == 6) {
                        log("ctlClientMsgHandler ---> Present CTL Lightness = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[1], msg.getBuffer()[0])) + " , Present CTL Temperature = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[3], msg.getBuffer()[2])) + " , Target CTL Lightness = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[5], msg.getBuffer()[4])));
                    } else if (msg.getBuffer().length == 8) {
                        log("ctlClientMsgHandler ---> Present CTL Lightness = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[1], msg.getBuffer()[0])) + " , Present CTL Temperature = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[3], msg.getBuffer()[2])) + " , Target CTL Lightness = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[5], msg.getBuffer()[4])) + " , Target CTL Temperature =  " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[7], msg.getBuffer()[6])));
                    } else if (msg.getBuffer().length == 9) {
                        log("ctlClientMsgHandler ---> Present CTL Lightness = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[1], msg.getBuffer()[0])) + " , presentCTLTemperature = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[3], msg.getBuffer()[2])) + " , Target CTL Lightness = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[5], msg.getBuffer()[4])) + " , Target CTL Temperature =  " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[7], msg.getBuffer()[6])) + " , Ramain Time = " +
                            MeshUtils.decimalToHexString("%02X", msg.getBuffer()[8]));
    ...
    private void hslClientMsgHandler(int modelHandle, BluetoothMeshAccessRxMessage msg) {
        int msgOpCode = msg.getOpCode();
        int src = msg.getSrcAddr();
        log("hslClientMsgHandler, msgOpCode = " + MeshUtils.decimalToHexString("%04X", msgOpCode) +
                " , src = " + MeshUtils.decimalToHexString("%04X", src) + " , isGroupSendMsg = " + isGroupSendMsg);
        if (isGroupSendMsg) { // group send msg
            groupSendMsgAck(src, msg);
        } else { // single node send msg
            if (ackNodeSendMsgList.contains(src)) {
                log("hslClientMsgHandler ---> already contain src");
                return;
            }
            MyApplication.sendMessageTime = System.currentTimeMillis() - MyApplication.sendMessageTime;
            ackNodeSendMsgList.add(src);
            ...
            switch (msg.getOpCode()) {
                case MeshConstants.MESH_MSG_LIGHT_HSL_STATUS: { // Light HSL Status
                    log("hslClientMsgHandler ---> opCode is MeshConstants.MESH_MSG_LIGHT_HSL_STATUS");
                    int[] opcodes = {MeshConstants.MESH_MSG_LIGHT_HSL_GET,
                        MeshConstants.MESH_MSG_LIGHT_HSL_SET};
                    removeResendMsg(opcodes, src);
                    if (msg.getBuffer().length == 2) {
                        log("hslClientMsgHandler ---> HSL Lightness = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[1], msg.getBuffer()[0])));
                    } else if (msg.getBuffer().length == 4) {
                        log("hslClientMsgHandler ---> HSL Lightness = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[1], msg.getBuffer()[0])) + " , HSL Hue = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[3], msg.getBuffer()[2])));
                    } else if (msg.getBuffer().length == 6) {
                        log("hslClientMsgHandler ---> HSL Lightness = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[1], msg.getBuffer()[0])) + " , HSL Hue = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[3], msg.getBuffer()[2])) + " , HSL Saturation = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[5], msg.getBuffer()[4])));
                    } else if (msg.getBuffer().length == 7) {
                        log("hslClientMsgHandler ---> HSL Lightness = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[1], msg.getBuffer()[0])) + " , HSL Hue = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[3], msg.getBuffer()[2])) + " , HSL Saturation = " +
                            MeshUtils.decimalToHexString("%04X", MeshUtils.getCombineNuber(msg.getBuffer()[5], msg.getBuffer()[4])) + " , Remaining Time = " +
                            MeshUtils.decimalToHexString("%04X", msg.getBuffer()[6]));
                    } else {
                        log("hslClientMsgHandler ---> No valid data , msg.getBuffer().length = " + msg.getBuffer().length);
                        return;
                    }
                    if (mBluetoothMeshListener != null) {
                        mBluetoothMeshListener.onNodeMsgRecieved(msg);
                ...
                case MeshConstants.MESH_MSG_LIGHT_HSL_RANGE_STATUS: { // Light HSL Range Status
                    log("hslClientMsgHandler ---> opCode is MeshConstants.MESH_MSG_LIGHT_HSL_RANGE_STATUS");
                    int[] opcodes = {MeshConstants.MESH_MSG_LIGHT_CTL_TEMPERATURE_RANGE_GET};
                    removeResendMsg(opcodes, src);
                    if (msg.getBuffer().length != 9) {
                        log("hslClientMsgHandler ---> No valid data , msg.getBuffer().length = " + msg.getBuffer().length);
                        return;
                    }
                    int lightHSLHueRangeMin = MeshUtils.getCombineNuber(msg.getBuffer()[2], msg.getBuffer()[1]);
                    int lightHSLHueRangeMax = MeshUtils.getCombineNuber(msg.getBuffer()[4], msg.getBuffer()[3]);
                    int lightHSLStaturationRangeMin = MeshUtils.getCombineNuber(msg.getBuffer()[6], msg.getBuffer()[5]);
                    int lightHSLStaturationRangeMax = MeshUtils.getCombineNuber(msg.getBuffer()[8], msg.getBuffer()[7]);
                    if (lightHSLHueRangeMin == -1 || lightHSLHueRangeMax == -1 ||
                            lightHSLStaturationRangeMin == -1 || lightHSLStaturationRangeMax == -1) {
                        log("hslClientMsgHandler ---> No valid data");
                        for (int i = 0; i < msg.getBuffer().length ; i++) {
                            log("hslClientMsgHandler ---> msg.getBuffer()[" + i + "] = " + MeshUtils.decimalToHexString("%02X", msg.getBuffer()[i]));
                        }
                        return;
                    }
                    log("hslClientMsgHandler ---> Status Code = " +
                        MeshUtils.decimalToHexString("%02X", msg.getBuffer()[0]) + " , Hue Range Min = " +
                        MeshUtils.decimalToHexString("%04X", lightHSLHueRangeMin) + " , Hue Range Max = " +
                        MeshUtils.decimalToHexString("%04X", lightHSLHueRangeMax) + " , Saturation Range Min = " +
                        MeshUtils.decimalToHexString("%04X", lightHSLStaturationRangeMin) + " , Saturation Range Min = " +
                        MeshUtils.decimalToHexString("%04X", lightHSLStaturationRangeMax));
                    if (mBluetoothMeshListener != null) {
                        mBluetoothMeshListener.onNodeMsgRecieved(msg);
                    }
                    break;
    ...
    private void parsePropertyFile() {
        ...
        try {
            ...
            if (null != p.getProperty("friship_netkey")) {
                frishipNetKey = p.getProperty("friship_netkey");
                log("friship_netkey=" + frishipNetKey);
            }
            if (null != p.getProperty("friship_appkey")) {
                frishipAppKey= p.getProperty("friship_appkey");
                log("friship_appkey=" + frishipAppKey);
            }
            int friship_device_name_number = 0;
            int friship_device_address_number = 0;
            if (null != p.getProperty("friship_device_name_number")) {
                friship_device_name_number = Integer.valueOf(p.getProperty("friship_device_name_number")).intValue();
                log("friship_device_name_number=" + friship_device_name_number);
            }
            if (null != p.getProperty("friship_device_address_number")) {
                friship_device_address_number = Integer.valueOf(p.getProperty("friship_device_address_number")).intValue();
                log("friship_device_address_number=" + friship_device_address_number);
            }
            for (int i = 0; i < friship_device_address_number; i++) {
                String friship_device_address = p.getProperty("friship_device_address_" + (i + 1));
                String friship_device_name = p.getProperty("friship_device_name_" + (i + 1));
                log("friship_device_address_" + (i + 1) + " = " + friship_device_address);
                log("friship_device_name_" + (i + 1) + " = " + friship_device_name);
                if (null != friship_device_address) {
                    frishipAddrList.add(friship_device_address);
                    frishipNameList.add(friship_device_name);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private BluetoothProfile.ServiceListener mServiceListener = new BluetoothProfile.ServiceListener() {
        ...
        public void onServiceConnected(int profile, BluetoothProfile proxy) {
            ...
            if(mBluetoothMeshListener != null) {
                //log("mBluetoothMeshListener != null");
                mBluetoothMeshListener.onBluetoothReset();
            }
        }
    ...
    public boolean onUnbind(Intent intent) {
        ...
        if(MyApplication.GattConnectStatus == MyApplication.PB_GATT_CONNECT) {
            ...
            MyApplication.GattConnectAddress = 0;
        }
    ...
    private void createMeshDevice() {
        ...
        int[] compDataHeader = {
            ...
            0x01, 0x00, // vid
            0xF4, 0x01, // crpl
            MyApplication.friship_mode ? (MeshConstants.MESH_FEATURE_PROXY | MeshConstants.MESH_FEATURE_FRIEND | MeshConstants.MESH_FEATURE_RELAY) : (MeshConstants.MESH_FEATURE_PROXY), // features
            0x00
        };
        if (MyApplication.friship_mode) {
            mBluetoothMesh.bearerAdvSetParams(200, 0x10, 0x10, 1, 50, 0x80, 0x60);
        } else {
    ...
    public int enableMeshFilter(boolean pbAdv, boolean meshMessage, boolean unprovBeacon, boolean secureBeacon){
        log("enableMeshFilter , pbAdv = " + pbAdv + " , meshMessage = " + meshMessage + " , unprovBeacon = " + unprovBeacon
            + " , secureBeacon = " + secureBeacon);
        return mBluetoothMesh.enableMeshFilter(pbAdv, meshMessage, unprovBeacon, secureBeacon);
    }

    public int enableMeshFilterWithNID(boolean pbAdv, boolean meshMessageWithNID, int[] meshMessageNetIndex,
        boolean unprovBeacon, boolean secureBeaconWithNetworkID, int[] secureBeaconNetIndex){
        log("enableMeshFilterWithNID: pbAdv = " + pbAdv + ", meshMessageWithNID = " + meshMessageWithNID
            + ", meshMessageNetIndex = " + Arrays.toString(meshMessageNetIndex) + ", unprovBeacon = " + unprovBeacon
            + ", secureBeaconWithNetworkID = " + secureBeaconWithNetworkID + ", secureBeaconNetIndex = " + Arrays.toString(secureBeaconNetIndex));
        return mBluetoothMesh.enableMeshFilterWithNetwork(pbAdv, meshMessageWithNID, meshMessageNetIndex,
                        unprovBeacon, secureBeaconWithNetworkID, secureBeaconNetIndex);
    }

    public int disableMeshFilter(){
        log("disableMeshFilter");
        return mBluetoothMesh.disableMeshFilter();
    }
    ...
    public boolean setBluetoothMeshEnabled(boolean enabled) {
        ...
        if(enabled) {
            ...
            if (isEnable) {// if mesh is already enabled, there is no onMeshEnabled event, we shall notify UI here
                log("mesh is already enabled, no onMeshEnabled event, only notify");
                if(mBluetoothMeshListener != null) {
            ...
            if (mBluetoothMesh != null && mBluetoothMesh.registerCallback(mBluetoothMeshCallback)) {
                mBluetoothMesh.enable(initParams);
            }
        } else {
            if (mBluetoothMesh != null) {
                mBluetoothMesh.unregisterCallback();
            }
            disable();
            log("disable");
            if(MyApplication.GattConnectStatus == MyApplication.PB_GATT_CONNECT) {
            ...
            mBluetoothMesh.disable();
            mBluetoothMesh.disableMeshFilter();
            isEnable = false;
        }
        return true;
    }
    ...
    public int inviteProvisioning(int bearer, int[] uuid, String addr, int addrType, String devName, int netkeyIndex, int appkeyIndex){
        ...
        if (bearer == MeshConstants.MESH_BEARER_GATT) {
            ...
            if (-1 != id) {
                currentProvNode.setProvisionTimerID(id);
                getTimerById(id).schedule(new ExTimerTask<Node>(5, currentProvNode), 60000); // adv provision time : 60s
            }
    ...
    private BluetoothMeshCallback mBluetoothMeshCallback = new BluetoothMeshCallback() {
        ...
        public void onMeshEnabled() {
            ...
            for(Map.Entry<Integer,Node> entry:mNodes.entrySet()) {
                Node node = entry.getValue();
                if(!node.isConfigSuccess()) { // if node config fail,not set timer for node
                    //log("onMeshEnabled config fail , address " + MeshUtils.decimalToHexString("%04X", node.getAddr()));
                    continue;
                }
                if(node.getCmdAddFriDev()) {
                    //log("onMeshEnabled friendship , address " + MeshUtils.decimalToHexString("%04X", node.getAddr()));
                    continue;
                }
                int i = createTimer();
                if(-1 != i && node.isConfigSuccess()) {
                    //log("onMeshEnabled config success , address " + MeshUtils.decimalToHexString("%04X", node.getAddr()));
                    node.setHeartBeatTimerID(i);
                    getTimerById(i).schedule(new ExTimerTask<Node>(2, node), 40000,40000);
                }
            }
        ...
        public void onScanUnProvDevice(int[] uuid, int oobInfom, int[] uriHash, int rssi) {
            super.onScanUnProvDevice(uuid, oobInfom, uriHash, rssi);
            //save ud for UI display
            log("onScanUnProvDevice, uuid = " + Arrays.toString(uuid) + " rssi = " + rssi);
            deleteLocalNodeByBear(MeshConstants.MESH_BEARER_ADV, uuid, null);
            if(mBluetoothMeshListener != null) {
                mBluetoothMeshListener.onScanUnProvDevice(uuid, oobInfom, uriHash, rssi);
            }
        ...
        public void onProvCapabilities(int numberOfElements, int algorithms, int publicKeyType, int staticOOBType, int outputOobSize, int outputOobAction, int inputOobSize, int inputOobAction) {
            ...
            log("onProvCapabilities ---> NumOfElements = " + numberOfElements);
            log("onProvCapabilities ---> Algorithms = " + algorithms);
            log("onProvCapabilities ---> PublicKeyType = " + publicKeyType);
            log("onProvCapabilities ---> StaticOobType = " + staticOOBType);
            log("onProvCapabilities ---> OutputOobSize = " + outputOobSize);
            log("onProvCapabilities ---> OutputOobAction = " + outputOobAction);
            log("onProvCapabilities ---> InputOobSize = " + inputOobSize);
            log("onProvCapabilities ---> InputOobAction = " + inputOobAction);
            ...
            */
            ...
            log("onProvCapabilities ---> startProvisioning with addr: " + MeshUtils.decimalToHexString("%04X", addr));
            log("onProvCapabilities ---> startProvisioning netkey = " + MeshUtils.intArrayToString(mAllNetKeyMap.get(currentProvNode.getCurrNetkeyIndex()).getValue(), true));
            currentProvNode.setAddr(addr);
        ...
        public void onProvDone(int address, int[] deviceKey, boolean success, boolean gatt_bearer) {
            ...
            if (address != currentProvNode.getAddr() || time >= 60) {
                log("provision done after provision timeout , or provisioning other device");
                return;
            }
            ...
            if (true == success) {
                ...
                //config relay set
                //configMsgRelaySet(currentProvNode, 1, 0, (int)(Math.random()*51));
                ...
                buffer.append(" , provision fail , time cost : ").append(time).append("s");
                log(buffer.toString());
        ...
        public void onBearerGattStatus(long handle, int status) {
            ...
            if (status == MeshConstants.MESH_BEARER_GATT_STATUS_CONNECTED) {
                if (currentProvNode.getConfigState() == MESH_NODE_CONFIG_STATE_PROVISIONING) { // gatt provisioning connect success
                    ...
                    MyApplication.GattConnectAddress = currentProvNode.getAddr();
                    notifyProvState(currentProvNode, true);
                    try {
                        if (getTimerById(currentProvNode.getProxyServiceTimerID()) != null) {
        ...
        @Override
        public void onFriendShipStatus(int address, int status, float timeCost) {
            Node node = getNodeByAddr(address);
            if (node == null) {
                log("onFriendShipStatus , but the node is not belong Node list , address: " + MeshUtils.decimalToHexString("%04X", address) + " , staus: " + getFriendShipStatus(status) + " , timeCost: " + timeCost);
                return ;
            }
            node.setFSStatus(status);
            if (status != MeshConstants.MESH_FRIENDSHIP_ESTABLISHED) {
                int fsContOff = node.getFSContOff();
                if (fsContOff >= 0xFFFF) {
                    fsContOff = 0;
                }
                fsContOff++;
                node.setFSContOff(fsContOff);
            }
            timeCost = (float)(Math.round(timeCost*1000)) / (1000);
            node.setFSOnCostTime(timeCost);
            log("onFriendShipStatus , address: " + MeshUtils.decimalToHexString("%04X", address) + " , staus: " + getFriendShipStatus(status) + " , timeCost: " + timeCost + " , Off Num: " + node.getFSContOff());
            //update UI here
            if (mBluetoothMeshListener != null) {
                mBluetoothMeshListener.onFriendShipStatus(node);
            }
        }
    };

    private String getFriendShipStatus(int status) {
        switch(status){
            case MeshConstants.MESH_FRIENDSHIP_TERMINATED:
                return "mesh friendship terminated";
            case MeshConstants.MESH_FRIENDSHIP_ESTABLISHED:
                return "mesh friendship established";
            case MeshConstants.MESH_FRIENDSHIP_ESTABLISH_FAILED:
                return "mesh friendship established failed";
            case MeshConstants.MESH_FRIENDSHIP_REQUEST_FRIEND_TIMEOUT:
                return "mesh friendship request friend timeout";
            case MeshConstants.MESH_FRIENDSHIP_SELECT_FRIEND_TIMEOUT:
                return "mesh friendship select friend timeout";
            default:
                log("getFriendShipStatus , status = " + status);
                return "unknown";
        }
    }
    ...
    public ArrayList<Integer> getNetKeyIndexs() {
        ArrayList<Integer> netKeyIndexs = new ArrayList<Integer>();
        for(Map.Entry<Integer, NetKey> entry:mAllNetKeyMap.entrySet()) {
            netKeyIndexs.add(entry.getKey());
        }
        return netKeyIndexs;
    }

    public HashMap<Integer, ArrayList<Integer>> getAppkeyIndexMaps() {
        HashMap<Integer, ArrayList<Integer>> appkeyIndexMaps = new HashMap<Integer, ArrayList<Integer>>();
        for(Map.Entry<Integer, NetKey> entry:mAllNetKeyMap.entrySet()) {
            appkeyIndexMaps.put(entry.getKey(), entry.getValue().getAppKeyList());
        }
        return appkeyIndexMaps;
    }
    ...
    public int addNewNetKey() {
        return addNewNetKey(null);
    }

    public int addNewNetKey(int[] netkey) {
        log("addNewNetKey ,mCurrMaxNetKeyIndex = " + mCurrMaxNetKeyIndex + " , netkey = " + MeshUtils.intArrayToString(netkey, " "));
        ...
        if ((mCurrMaxNetKeyIndex == 0) && (this.mAllNetKeyMap.size() == 0)) { //primay netkey, generic a random value
            ...
            if (netkey != null && netkey.length == 16) {
                System.arraycopy(netkey, 0, keyValue, 0, 16);
            } else {
            ...
            if (isEnable()) {   //If enabled, we shall add the key to SDK, otherwise, the key will be added on enabled
                if (0 == mBluetoothMesh.setNetkey(MeshConstants.MESH_KEY_OP_ADD, keyValue, (mCurrMaxNetKeyIndex+1))) {
                    ...
                    if (mBluetoothMeshListener != null) {
                        mBluetoothMeshListener.addNewNetKey(true, (mCurrMaxNetKeyIndex + 1));
                    }
                } else {
                    log("Add netkey " + (mCurrMaxNetKeyIndex + 1) + ", value: " + MeshUtils.intArrayToString(keyValue, false) + ", fail");
                    if (mBluetoothMeshListener != null) {
                        mBluetoothMeshListener.addNewNetKey(false, (mCurrMaxNetKeyIndex + 1));
                    }
                    return -1;
                }
            }
    ...
    public int addNewAppKey(int netkeyIndex) {
        return addNewAppKey(netkeyIndex, null);
    }

    public int addNewAppKey(int netkeyIndex, int[] appkey) {
        log("addNewAppKey, mCurrMaxAppKeyIndex = " + mCurrMaxAppKeyIndex + ", netkeyIndex = " + netkeyIndex +
                    " , appkey = " + MeshUtils.intArrayToString(appkey, " "));
        ...
        if ((mCurrMaxAppKeyIndex == 0) && (this.mAllAppKeyMap.size() == 0)) { //first appkey, generic a random value
            ...
            if (appkey != null && appkey.length == 16) {
                System.arraycopy(appkey, 0, keyValue, 0, 16);
            } else {
                System.arraycopy(mMyUUID, 0, keyValue, 0, 16);
            ...
            if (isEnable()) {   //If enabled, we shall add the key to SDK, otherwise, the key will be added on enabled
                if (0 == mBluetoothMesh.setAppkey(MeshConstants.MESH_KEY_OP_ADD, keyValue, netkeyIndex, (mCurrMaxAppKeyIndex+1))) {
                    ...
                    if (mBluetoothMeshListener != null) {
                        mBluetoothMeshListener.addNewAppKey(true, netkeyIndex, (mCurrMaxAppKeyIndex + 1));
                    }
                } else {
                    log("Add appkey " + (mCurrMaxAppKeyIndex+1) + ", value: " + MeshUtils.intArrayToString(keyValue, false) + ", fail");
                    if (mBluetoothMeshListener != null) {
                        mBluetoothMeshListener.addNewAppKey(false, netkeyIndex, (mCurrMaxAppKeyIndex + 1));
                    }
                    return -1;
                }
            }
        ...
        log("addNewAppKey, index = " + mCurrMaxAppKeyIndex + ", value = " + MeshUtils.intArrayToString(keyValue, false) + ", with netkeyIndex = " + netkeyIndex);
        key.setAppKeyMeta(mCurrMaxAppKeyIndex, keyValue, netkeyIndex);
        this.mAllAppKeyMap.put(mCurrMaxAppKeyIndex, key);
        this.mAllNetKeyMap.get(netkeyIndex).addAppkeyIndex(mCurrMaxAppKeyIndex);
    ...
    public Node getNodeByGattAddr(String address) {
        ...
        for(Map.Entry<Integer,Node> entry:mNodes.entrySet()) {
            Node node = entry.getValue();
            if (address.equals(node.getGattAddr())) {
                return node;
            }
        }
        return null;
    ...
    public HashMap<Integer, NetKey> getNetKeyMap() {
        return mAllNetKeyMap;
    }

    public HashMap<Integer, AppKey> getAppKeyMap() {
        return mAllAppKeyMap;
    }
    ...
    public boolean removeNode(Node node, boolean bNodeReset) {
        ...
        if (null == nodeMap) {
            log("removeNode, warning, nodeMap is null, this shall not happen...");
            ...
            log("removeNode, nodeMap size for netkeyIndex " + MeshUtils.decimalToHexString("%04X", node.getCurrNetkeyIndex()) + " is " + nodeMap.size());
            ArrayList<Node> nodeList = new ArrayList<Node>(nodeMap.values());
            for (int i = 0; i < nodeList.size(); i++) {
                if (nodeList.get(i).getAddr() == node.getAddr()) {
                    log("removeNode, remove node " + MeshUtils.decimalToHexString("%04X", node.getAddr()) + " from whitelist");
                    nodeList.remove(i);
                    break;
                }
            }
            ...
            log("removeNode, nodeList size " + nodeList.size());
            if (nodeList.size() == 0) {
                keyRefreshStart(node.getCurrNetkeyIndex(), null);
            } else {
    ...
    public void sendPacket(int dst, int src, int ttl, int netKeyIndex, int appKeyIndex, int[] payload) {
        if (sendPacketTimer != null) {
            log("sendPacket , maybe sending Light HSL msg , please wait");
            return;
        }
        sendPacketTimer = new Timer();
        sendPacketTimer.schedule(new TimerTask() {

            @Override
            public void run() {
                log(new StringBuilder().append("sendPacket , dst = ").append(dst)
                          .append(" , src = ").append(src)
                          .append(" , ttl = ").append(ttl)
                          .append(" , netKeyIndex = ").append(MeshUtils.decimalToHexString("%04X", netKeyIndex))
                          .append(" , appKeyIndex = ").append(MeshUtils.decimalToHexString("%04X", appKeyIndex))
                          .append(" , payload = ").append(MeshUtils.intArrayToString(payload, ""))
                          .toString());
                mBluetoothMesh.sendPacket(dst, src, ttl, netKeyIndex, appKeyIndex, payload);
                sendPacketCount++;
                if (sendPacketCount == 5) {
    ...
    public void sendMessage(MeshMessage msg, Node node, int appKeyIndex, int groupAddr, int[] payload) {
        ...
        log("sendMessage  , opcode = " + MeshUtils.decimalToHexString("%04X", msg.getOpCode())+ ", groupAddr = " + MeshUtils.decimalToHexString("%04X", groupAddr) +
            " , netkeyIndex = " + node.getCurrNetkeyIndex() + " , appkeyIndex = " + appKeyIndex +
        ...
        if (isGroupSendMsging) {
            log("sendMessage ---> group sending msg state, return!!");
            return;
        }
        ...
        if (dst == 0xFFFF) { // group send msg
            if (msg.getOpCode() == MeshConstants.MESH_MSG_GENERIC_ONOFF_SET_UNRELIABLE ||
                msg.getOpCode() == MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_SET_UNACKNOWLEDGED ||
                msg.getOpCode() == MeshConstants.MESH_MSG_LIGHT_CTL_TEMPERATURE_SET_UNACKNOWLEDGED ||
                msg.getOpCode() == MeshConstants.MESH_MSG_LIGHT_CTL_SET_UNACKNOWLEDGED) {
                log("sendMessage ---> send unack msg");
            } else {
                isGroupSendMsg = true;
                ...
                for(Map.Entry<Integer,Node> entry:mNodes.entrySet()) {
                    if (entry.getValue().getCmdAddFriDev()) {
                        continue;
                    }
                ...
                log("sendMessage ---> all node size = " + mNodes.size() + " , config success node size = " + cfSuccessNodeList.size());
        ...
        if (-1 != id) {
            ...
            if (isGroupSendMsg) {
                ...
                getTimerById(id).schedule(new ExTimerTask<Node>(7, currentProvNode), 2000); // send message timeout : 2s , unack msg also need timer, set meshmode standy after timeout
            }
        }
        ...
        switch (msg.getOpCode()) {
            case MeshConstants.MESH_MSG_GENERIC_ONOFF_GET: { //Generic OnOff  Get
                log("sendMessage ---> genericOnOffGet");
                ...
                mGenericOnOffClient.genericOnOffGet();
                addResendMsg(node, payload, 0, 0, 0, MeshConstants.MESH_MSG_GENERIC_ONOFF_GET, SEND_MSG_DEFAULT_RESEND_TIMES);
                break;
            }
            case MeshConstants.MESH_MSG_GENERIC_ONOFF_SET: //Generic OnOff  Set
                log("sendMessage ---> genericOnOffSet , OnOff = " + payload[0]);
                mGenericOnOffClient.setTxMessageHeader(parseAddressType(dst),
                                                    dst,
                                                    null,
                                                    mPrimaryElement.getAddress(),
                ...
                addResendMsg(node, payload, 0, 0, 0, MeshConstants.MESH_MSG_GENERIC_ONOFF_SET, SEND_MSG_DEFAULT_RESEND_TIMES);
                break;
            case MeshConstants.MESH_MSG_GENERIC_ONOFF_SET_UNRELIABLE: { //Generic OnOff  Set Unack
                log("sendMessage ---> genericOnOffSetUnack , OnOff = " + payload[0]);
                mGenericOnOffClient.setTxMessageHeader(parseAddressType(dst),
                                                    dst,
                                                    null,
                                                    mPrimaryElement.getAddress(),
                ...
                addResendMsg(node, payload, 0, 0, 0, MeshConstants.MESH_MSG_GENERIC_ONOFF_SET_UNRELIABLE, SEND_MSG_DEFAULT_RESEND_TIMES);
                break;
            }
            ...
            case MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_GET: { //Light Lightness  Get
                log("sendMessage ---> lightLightnessGet");
                mLightnessClient.setTxMessageHeader(parseAddressType(dst),
                                                    dst,
                                                    null,
                                                    mPrimaryElement.getAddress(),
                                                    defaultTTL, node.getCurrNetkeyIndex(), appKeyIndex, msg.getOpCode());
                mLightnessClient.lightLightnessGet();
                addResendMsg(node, payload, 0, 0, 0, MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_GET, SEND_MSG_DEFAULT_RESEND_TIMES);
                break;
            }
            case MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_SET: { //Light Lightness  Set
                log("sendMessage ---> lightLightnessSet , lightness = " + MeshUtils.decimalToHexString("%04X", payload[0]));
                mLightnessClient.setTxMessageHeader(parseAddressType(dst),
                                                    dst,
                                                    null,
                                                    mPrimaryElement.getAddress(),
                                                    defaultTTL, node.getCurrNetkeyIndex(), appKeyIndex, msg.getOpCode());
                mLightnessClient.lightLightnessSet(payload[0], lightnessTID++, 0, 0);
                addResendMsg(node, payload, 0, 0, 0, MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_SET, SEND_MSG_DEFAULT_RESEND_TIMES);
                break;
            }
            case MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_SET_UNACKNOWLEDGED: { //Light Lightness  Set Unack
                log("sendMessage ---> lightLightnessSetUnack , lightness = " + MeshUtils.decimalToHexString("%04X", payload[0]));
                mLightnessClient.setTxMessageHeader(parseAddressType(dst),
                                                    dst,
                                                    null,
                                                    mPrimaryElement.getAddress(),
                                                    defaultTTL, node.getCurrNetkeyIndex(), appKeyIndex, msg.getOpCode());
                mLightnessClient.lightLightnessSetUnacknowledged(payload[0], lightnessTID++, 0, 0);
                addResendMsg(node, payload, 0, 0, 0, MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_SET_UNACKNOWLEDGED, SEND_MSG_DEFAULT_RESEND_TIMES);
                break;
            }
            case MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_RANGE_GET: { //Light Lightness Range Get
                log("sendMessage ---> lightLightnessRangeGet");
                mLightnessClient.setTxMessageHeader(parseAddressType(dst),
                                                    dst,
                                                    null,
                                                    mPrimaryElement.getAddress(),
                                                    defaultTTL, node.getCurrNetkeyIndex(), appKeyIndex, msg.getOpCode());
                mLightnessClient.lightLightnessRangeGet();
                addResendMsg(node, payload, 0, 0, 0, MeshConstants.MESH_MSG_LIGHT_LIGHTNESS_RANGE_GET, SEND_MSG_DEFAULT_RESEND_TIMES);
                break;
            }
            case MeshConstants.MESH_MSG_LIGHT_CTL_TEMPERATURE_GET: { //Light CTL Temperature Get
                log("sendMessage ---> lightCtlTemperatureGet");
                mCTLClient.setTxMessageHeader(parseAddressType(dst),
                                                    dst,
                                                    null,
                                                    mPrimaryElement.getAddress(),
                                                    defaultTTL, node.getCurrNetkeyIndex(), appKeyIndex, msg.getOpCode());
                mCTLClient.lightCtlTemperatureGet();
                addResendMsg(node, payload, 0, 0, 0, MeshConstants.MESH_MSG_LIGHT_CTL_TEMPERATURE_GET, SEND_MSG_DEFAULT_RESEND_TIMES);
                break;
            }
            case MeshConstants.MESH_MSG_LIGHT_CTL_TEMPERATURE_SET: { //Light CTL Temperature Set
                log("sendMessage ---> lightCtlTemperatureSet , ctlTemperature = "
                    + MeshUtils.decimalToHexString("%04X", payload[0]) + " , ctlDeltaUV = "
                    + MeshUtils.decimalToHexString("%04X", payload[1]));
                mCTLClient.setTxMessageHeader(parseAddressType(dst),
                                                    dst,
                                                    null,
                                                    mPrimaryElement.getAddress(),
                                                    defaultTTL, node.getCurrNetkeyIndex(), appKeyIndex, msg.getOpCode());
                mCTLClient.lightCtlTemperatureSet(payload[0], payload[1], ctlTemperatureTID++, 0, 0);
                addResendMsg(node, payload, 0, 0, 0, MeshConstants.MESH_MSG_LIGHT_CTL_TEMPERATURE_SET, SEND_MSG_DEFAULT_RESEND_TIMES);
                break;
            }
            case MeshConstants.MESH_MSG_LIGHT_CTL_TEMPERATURE_SET_UNACKNOWLEDGED: { //Light CTL Temperature Set Unack
                log("sendMessage ---> lightCtlTemperatureSetUnacknowledged , ctlTemperature = "
                    + MeshUtils.decimalToHexString("%04X", payload[0]) + " , ctlDeltaUV = "
                    + MeshUtils.decimalToHexString("%04X", payload[1]));
                mCTLClient.setTxMessageHeader(parseAddressType(dst),
                                                    dst,
                                                    ...
                                                    mPrimaryElement.getAddress(),
                                                    defaultTTL, node.getCurrNetkeyIndex(), appKeyIndex, msg.getOpCode());
                mCTLClient.lightCtlTemperatureSetUnacknowledged(payload[0], payload[1], ctlTemperatureTID++, 0, 0);
                addResendMsg(node, payload, 0, 0, 0, MeshConstants.MESH_MSG_LIGHT_CTL_TEMPERATURE_SET_UNACKNOWLEDGED, SEND_MSG_DEFAULT_RESEND_TIMES);
                break;
            }
            case MeshConstants.MESH_MSG_LIGHT_CTL_TEMPERATURE_RANGE_GET: { //Light CTL Temperature Range Get
                log("sendMessage ---> lightCtlTemperatureRangeGet");
                mCTLClient.setTxMessageHeader(parseAddressType(dst),
                                                    dst,
                                                    null,
                                                    mPrimaryElement.getAddress(),
                                                    defaultTTL, node.getCurrNetkeyIndex(), appKeyIndex, msg.getOpCode());
                mCTLClient.lightCtlTemperatureRangeGet();
                addResendMsg(node, payload, 0, 0, 0, MeshConstants.MESH_MSG_LIGHT_CTL_TEMPERATURE_RANGE_GET, SEND_MSG_DEFAULT_RESEND_TIMES);
                break;
            }
            case MeshConstants.MESH_MSG_LIGHT_CTL_DEFAULT_GET: { //Light CTL Default Get
                log("sendMessage ---> lightCtlDefaultGet");
                mCTLClient.setTxMessageHeader(parseAddressType(dst),
                                                    dst,
                                                    null,
                                                    mPrimaryElement.getAddress(),
                                                    defaultTTL, node.getCurrNetkeyIndex(), appKeyIndex, msg.getOpCode());
                mCTLClient.lightCtlDefaultGet();
                addResendMsg(node, payload, 0, 0, 0, MeshConstants.MESH_MSG_LIGHT_CTL_DEFAULT_GET, SEND_MSG_DEFAULT_RESEND_TIMES);
                break;
            }
            case MeshConstants.MESH_MSG_LIGHT_CTL_GET: { //Light CTL Get
                log("sendMessage ---> lightCtlGet");
                mCTLClient.setTxMessageHeader(parseAddressType(dst),
                                                    dst,
                                                    null,
                                                    mPrimaryElement.getAddress(),
                                                    defaultTTL, node.getCurrNetkeyIndex(), appKeyIndex, msg.getOpCode());
                mCTLClient.lightCtlGet();
                addResendMsg(node, payload, 0, 0, 0, MeshConstants.MESH_MSG_LIGHT_CTL_GET, SEND_MSG_DEFAULT_RESEND_TIMES);
                break;
            }
            case MeshConstants.MESH_MSG_LIGHT_CTL_SET: { //Light CTL Set
                log("sendMessage ---> lightCtlSet , ctlLightness = "
                    + MeshUtils.decimalToHexString("%04X", payload[0]) + " , ctlTemperature = "
                    + MeshUtils.decimalToHexString("%04X", payload[1]) + " , ctlDeltaUV = "
                    + MeshUtils.decimalToHexString("%04X", payload[2]));
                mCTLClient.setTxMessageHeader(parseAddressType(dst),
                                                    dst,
                                                    null,
                                                    mPrimaryElement.getAddress(),
                                                    defaultTTL, node.getCurrNetkeyIndex(), appKeyIndex, msg.getOpCode());
                mCTLClient.lightCtlSet(payload[0], payload[1], payload[2], ctlTID++, 0, 0);
                addResendMsg(node, payload, 0, 0, 0, MeshConstants.MESH_MSG_LIGHT_CTL_SET, SEND_MSG_DEFAULT_RESEND_TIMES);
                break;
            }
            case MeshConstants.MESH_MSG_LIGHT_CTL_SET_UNACKNOWLEDGED: { //Light CTL Set Unack
                log("sendMessage ---> lightCtlSetUnacknowledged , ctlLightness = "
                    + MeshUtils.decimalToHexString("%04X", payload[0]) + " , ctlTemperature = "
                    + MeshUtils.decimalToHexString("%04X", payload[1]) + " , ctlDeltaUV = "
                    + MeshUtils.decimalToHexString("%04X", payload[2]));
                ...
                mCTLClient.setTxMessageHeader(parseAddressType(dst),
                                                    ...
                                                    mPrimaryElement.getAddress(),
                                                    defaultTTL, node.getCurrNetkeyIndex(), appKeyIndex, msg.getOpCode());
                mCTLClient.lightCtlSetUnacknowledged(payload[0], payload[1], payload[2], ctlTID++, 0, 0);
                addResendMsg(node, payload, 0, 0, 0, MeshConstants.MESH_MSG_LIGHT_CTL_SET_UNACKNOWLEDGED, SEND_MSG_DEFAULT_RESEND_TIMES);
                break;
            }
            ...
            case MeshConstants.MESH_MSG_LIGHT_HSL_GET: { //Light HSL Get
                log("sendMessage ---> lightHslGet");
                mHSLClient.setTxMessageHeader(parseAddressType(dst),
                                                    dst,
                                                    null,
                                                    mPrimaryElement.getAddress(),
                                                    defaultTTL, node.getCurrNetkeyIndex(), appKeyIndex, msg.getOpCode());
                mHSLClient.lightHslGet();
                addResendMsg(node, payload, 0, 0, 0, MeshConstants.MESH_MSG_LIGHT_HSL_GET, SEND_MSG_DEFAULT_RESEND_TIMES);
                break;
            }
            case MeshConstants.MESH_MSG_LIGHT_HSL_SET: { //Light HSL Set
                log("sendMessage ---> lightHslSet , hslLightness = "
                    + MeshUtils.decimalToHexString("%04X", payload[0]) + " , hslHue = "
                    + MeshUtils.decimalToHexString("%04X", payload[1]) + " , hslSaturation = "
                    + MeshUtils.decimalToHexString("%04X", payload[2]));
                mHSLClient.setTxMessageHeader(parseAddressType(dst),
                                                    dst,
                                                    null,
                                                    mPrimaryElement.getAddress(),
                                                    defaultTTL, node.getCurrNetkeyIndex(), appKeyIndex, msg.getOpCode());
                mHSLClient.lightHslSet(payload[0], payload[1], payload[2], hslTID++, 0, 0);
                addResendMsg(node, payload, 0, 0, 0, MeshConstants.MESH_MSG_LIGHT_HSL_SET, SEND_MSG_DEFAULT_RESEND_TIMES);
                break;
            }
            case MeshConstants.MESH_MSG_LIGHT_HSL_SET_UNACKNOWLEDGED: { //Light HSL Set Unack
                log("sendMessage ---> lightHslSetUnacknowledged , hslLightness = "
                    + MeshUtils.decimalToHexString("%04X", payload[0]) + " , hslHue = "
                    + MeshUtils.decimalToHexString("%04X", payload[1]) + " , hslSaturation = "
                    + MeshUtils.decimalToHexString("%04X", payload[2]));
                mHSLClient.setTxMessageHeader(parseAddressType(dst),
                                                    ...
                                                    mPrimaryElement.getAddress(),
                                                    defaultTTL, node.getCurrNetkeyIndex(), appKeyIndex, msg.getOpCode());
                mHSLClient.lightHslSetUnacknowledged(payload[0], payload[1], payload[2], hslTID++, 0, 0);
                addResendMsg(node, payload, 0, 0, 0, MeshConstants.MESH_MSG_LIGHT_HSL_SET_UNACKNOWLEDGED, SEND_MSG_DEFAULT_RESEND_TIMES);
                break;
            }
            case MeshConstants.MESH_MSG_LIGHT_HSL_RANGE_GET: { //Light HSL Range Get
                log("sendMessage ---> lightHslRangeGet");
                mHSLClient.setTxMessageHeader(parseAddressType(dst),
                                                    dst,
                                                    null,
                                                    mPrimaryElement.getAddress(),
                                                    defaultTTL, node.getCurrNetkeyIndex(), appKeyIndex, msg.getOpCode());
                mHSLClient.lightHslRangeGet();
                addResendMsg(node, payload, 0, 0, 0, MeshConstants.MESH_MSG_LIGHT_HSL_RANGE_GET, SEND_MSG_DEFAULT_RESEND_TIMES);
                break;
            }
